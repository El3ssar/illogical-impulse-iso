...
# Neutralise ALL stock alpm hooks for THIS box. When `pacman -S` installs the
# demo members it fires post-transaction hooks; in a non-booted nspawn container
# (we run a one-shot /bin/bash, not --boot, so there is no PID-1 systemd / system
# bus) the systemd/dbus-touching hooks (daemon-reload, enqueue-marked, tmpfiles,
# udev-reload, dbus-reload, …) block trying to reach the manager and HANG the
# transaction. pacman's HookDir only ADDS dirs (the system hooks always run), so
# hooks can only be disabled by shadowing each by NAME with a /dev/null symlink in
# the override dir. Shadow every stock hook (version-robust — enumerated, not a
# fixed list). Disposable-box only (evaporates on exit); the demo members (sl,
# cowsay) are plain CLIs whose ledger-tracked removal needs none of these hooks.
install -d /etc/pacman.d/hooks
shopt -s nullglob
for _hk in /usr/share/libalpm/hooks/*.hook; do
  ln -sf /dev/null "/etc/pacman.d/hooks/$(basename "$_hk")"
done
shopt -u nullglob
