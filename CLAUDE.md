...
| Add/extend the graphical Control Center (GUI, #14) | `overlay/airootfs/usr/share/illogical-impulse/control/` — a **standalone** `qs -p` config (clone `welcome/shell.qml`; **ZERO** `quickshell/ii` imports) that is the GUI home for every reversible `iictl` tweak, DISTINCT from upstream's `Super+I` rice settings. Add a pane = `panes/<X>.qml` (an `Item` root composing `control/_ui/` + reading the `Colors`/`Ctl` singletons) **+ one descriptor line in `control/panes.js`** — the rail + `StackLayout` are registry-generated, so `shell.qml` needs NO edit (the GUI analogue of `iictl.d/<verb>`). A pure `--spec` domain is a one-liner: `SpecForm { domain: "<verb>" }` (the generic choice/list/toggle chooser renderer, the same contract as `iictl-tui`). Every mutation goes through `Ctl` → `iictl <verb>` (no pane touches the filesystem; each verb records its own ledger row, so the GUI records nothing itself). `Colors.qml` watches `generated/colors.json` READ-ONLY (live re-theme, static fallback). Launched four ways: `iictl center` (drop-in `scripts/runtime-lib/iictl.d/center`), the `.desktop` (`System;Settings;`), the welcome-card button, and the `SUPER+SHIFT+I` / `SUPER+ALT+C` fenced keybind (audit-before-reserve: `SUPER+ALT+Space` is upstream's Float/Tile). `validate.sh` guards it — including a chord-collision check against upstream (#14). |
| GTK/icon/cursor coherence (#26) | `overlay/skel-distro/.config/gtk-3.0/settings.ini` + `gtk-4.0/settings.ini` (theme/icon/cursor **NAMES**: adw-gtk3 / Papirus-Dark / Bibata-Modern-Classic) + `overlay/skel-distro/.icons/default/index.theme` (`Inherits=Bibata…`). ORTHOGONAL to matugen — it owns the COLOR `gtk-3.0/gtk.css` (its `rsync --delete` template output); NEVER bake `gtk.css`. Packages `adw-gtk3`, `papirus-icon-theme` (official), `bibata-cursor-theme` (AUR) under the `# theme coherence` block in `packages/goodies.list`. Installed-user only (not seeded to liveuser). Revert = delete → GTK falls back to system defaults, matugen colours persist. `validate.sh` guards it (#26a/b). |
| Wallpaper: set/pack/video (#26) | `scripts/runtime-lib/iictl.d/wallpaper` (`get`/`set`→`switchwall.sh --image`/`pack`/`video`/`list`/`--revert`/`--spec`). Curated pack is **FETCH-ONLY**: only the manifest `overlay/airootfs/usr/share/illogical-impulse/wallpapers/pack.list` (`filename url sha256 [rev]`) rides in the squashfs; `iictl wallpaper pack` downloads into `~/Pictures/Wallpapers/showcase/` (where upstream's chooser looks), sha256-verifies each, ledger-records the fetched set (kind=path). Never bake images. Revert (`iictl revert-all wallpaper:`) removes only the fetched set; the single baked default (70-assets) stays. `validate.sh` guards manifest shape + 64-hex sha256 + no-baked-images (#26c). |
| Material You feeder bridge (nvim/IDE/terminals, #26) | distro STANDALONE matugen config + templates under UNOWNED `overlay/skel-distro/.config/illogical-impulse-theming/` (`config.toml` + `templates/{nvim,terminal,ide}/…`). Hook `overlay/skel-distro/.config/hypr/custom/scripts/ii-theme-feeder.sh` reads the FINISHED `~/.local/state/quickshell/user/generated/colors.json` READ-ONLY and runs `matugen json <colors.json> --config <ours>` (NEVER upstream's matugen `config.toml`). `iictl theme feeder {enable,disable,run,status}` (in `scripts/runtime-lib/iictl.d/theme`) wires a SEPARATE sentinel-fenced `theme-feeder` block into `custom/execs.lua` (baked COMMENTED/OFF; enable uncomments via `ii_lua_block_write` + ledger). `validate.sh` guards it (#26d). |
| Plymouth opt-in (never baked, #26) | `iictl theme plymouth {enable,disable,regen,status}` (in `scripts/runtime-lib/iictl.d/theme`). enable installs `plymouth` ONLINE, adds the `plymouth` HOOK, regenerates BOTH mkinitcpio presets by reusing `scripts/runtime/ii-prepare-bootloader`'s `ii_regen_both_presets` (sourced lib-only via `II_BOOTLOADER_LIB_ONLY=1` — NO duplicated HOOKS logic). disable/`revert-all` restore the recorded prior HOOKS line + regen both. `iictl theme doctor` flags a `plymouth` HOOK without a regenerated initramfs. `plymouth` is in NO baked package list — `validate.sh` guards it (#26e / black-screen §6). |
...
- **Plymouth + universal initramfs** → black screens. Not shipped; opt-in only.
  `plymouth` is in NO baked package list; `iictl theme plymouth enable` installs it
  ONLINE, adds the `plymouth` HOOK, and regenerates BOTH mkinitcpio presets by
  reusing `ii-prepare-bootloader`'s `ii_regen_both_presets` (sourced lib-only via
  `II_BOOTLOADER_LIB_ONLY=1` — the SINGLE both-preset path, never a duplicated
  HOOKS list). `disable`/`revert-all` restore the recorded prior HOOKS line + regen
  both; `iictl theme doctor` flags a `plymouth` HOOK without a regenerated
  initramfs (the black-screen trap). `validate.sh` guards no-baked-plymouth + the
  both-preset reuse + the revert inverse (#26e). Don't bake plymouth or drop the
  both-preset regen.
...
- **Control Center keybind is BAKED (always-on), the widget leader is RUNTIME
  (opt-in) — don't unify them** → the Control Center (#14) is a core, always-present
  feature, so its `SUPER+SHIFT+I`/`SUPER+ALT+C` bind is BAKED into
  `overlay/skel-distro/.config/hypr/custom/keybinds.lua` as a sentinel-fenced
  `control-center` block on top of upstream's stock edit-keybinds line (the slot is
  a single-file `require("custom.keybinds")`, NOT globbed, so the whole file must
  reproduce upstream's stub verbatim — stripping the fence yields upstream's stub
  byte-for-byte; `validate.sh` guards this like the welcome execs.lua fence). Because
  a baked skel fence never passes through `ii_lua_block_write`, `ii-post-install`
  records the matching `lua-block` ledger row itself (AS the new user via `runuser`,
  INST-05) so `iictl revert-all` strips it → vanilla. This is the SAME pattern as the
  welcome execs.lua fence (REV-01) and the OPPOSITE of the widget leader (#20), which
  is written at RUNTIME by `iictl widget enable` precisely because widgets are
  dormant/opt-in. The #20 `validate.sh` guard forbids only a *widget* fence in skel
  (`-- >>> illogical-impulse widgets`), NOT the always-on `control-center` fence.
  Don't move the CC bind to runtime or the widget leader to baked — the baked-vs-
  runtime split tracks always-on-vs-opt-in. Chords were audited-before-reserved
  (#20 lesson): `SUPER+SHIFT+I` is free (upstream binds only `SUPER+I`), and the hub
  is `SUPER+ALT+C` — NOT `SUPER+ALT+Space`, which upstream already binds to Window
  Float/Tile (#14's "unbound" claim was stale). `validate.sh` diffs the CC-fenced
  binds against upstream's `hyprland/keybinds.lua` so a colliding chord fails.
