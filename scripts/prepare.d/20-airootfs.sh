...
# iictl framework: shared header + ledger (sourced libs, 0644) and the iictl.d/
# drop-in subcommands (executable, 0755). These ride the survive-path — ii-verify
# purges /usr/local/lib/ii by named file, keeping iictl.d/ + ledger.sh so the
# installed system's iictl plugin layer works (PROPOSAL §4 Pillar 0/3).
...
ok "iictl framework staged ($(find "$BUILD/airootfs/usr/local/lib/ii/iictl.d" -maxdepth 1 -type f | wc -l) iictl.d/ plugin(s) + header + ledger)"
